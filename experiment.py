...
class Experiment(object):
    ...
    def _preprocessing(self, directory, train):
        ...
        topk, indices = torch.topk(torch.tensor([torch.mean(cluster_centers[cluster_ids_x==i].float()) for i in range(8)]),2)#torch.tensor([(cluster_ids_x==i).nonzero().size(0) for i in range(8)]), 1)
