...
class Experiment(object):
    ...
    def _preprocessing(self, train):
        ...
        if train:
            ...
            _, indices = torch.topk(torch.tensor([(cluster_ids_x==i).nonzero().size(0) for i in range(best_k)]), best_k-1)
            indices = torch.cat([(cluster_ids_x==i).nonzero() for i in indices], dim=0).view(-1).tolist()
            print(f"\n\nResult of k-means on {best_k} clusters: {len(indices)} of {X.size(0)} samples remain, taken from top {best_k-1} cluster(s) according to mode.\n\n")
