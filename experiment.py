...
class Experiment(object):
    ...
    def _run(self, dataset, config: dict):
        ...
        while (self.classifier.curr_epoch < init_epoch + config["epochs"]):
            ...
            score_, k_, indices_ = self._features_selection(loaders[random.randrange(0,2)], K, score)
            if score < score_:
                #K = K // 2 if K > 2 else 8
            ...
            f1_train, f1_val, acc_train, acc_val, loss_train, loss_val = self.classifier._run_epoch(loaders, indices if self.classifier.library != "timm" else indices.view(self.classifier.bs, 3, 32, 32), k)
    ...
    def _features_selection(self, loader, K, score):
        ...
        while max(list(memoisation.keys())) != score or score is float("-inf") or score is float("nan"):
            data = next(iter(loader))
            X = data["input_ids"].cpu().numpy() if self.classifier.library != "timm" else data["input_ids"].view(self.classifier.bs, -1).cpu().numpy()
            Z = torch.tensor(X.T)
            iterations += 1
