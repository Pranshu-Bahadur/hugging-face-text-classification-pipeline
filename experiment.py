...
class Experiment(object):
    ...
    def _preprocessing(self, directory, train):
        ...
        X = X.view(X.size(0), -1)
        Y = torch.tensor(np.asarray(dataSetFolder.dataset["type"].values)).view(Y.size(0), -1).cuda()
        XY = torch.cat((X,Y)).cuda()
        cluster_ids_x, cluster_centers = kmeans(X=XY, num_clusters=8, device=torch.device('cuda:0'), distance='soft_dtw')
