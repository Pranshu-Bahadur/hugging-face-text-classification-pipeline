...
class Experiment(object):
    ...
    def _preprocessing(self, directory, train):
        ...
        cluster_ids_x, cluster_centers = kmeans(X=XY, num_clusters=8, device=torch.device('cuda:0'))
        topk, indices = torch.topk(torch.mean(torch.cat([cluster_ids_x[cluster_ids_x==i] for i in range(8)], dim=1).float()),2)#torch.tensor([(cluster_ids_x==i).nonzero().size(0) for i in range(8)]), 1)
