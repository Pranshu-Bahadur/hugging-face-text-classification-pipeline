...
class Experiment(object):
    ...
    def finding_k(self, X, n):
        ...
        for k in range(2, n+1):
            ...
            print(cluster_ids)
            curr_inertia = sum([(1/(2*i+1))*torch.sum((X[(cluster_ids==i).nonzero()].cpu() - centers[i].cpu() if centers[i].size(0) != 0 else torch.zeros((cluster_ids==i).nonzero().size(0)))**2, dim=-1) for i in range(k)])
            print(curr_inertia)
            if k!=2:
