...
class NLPClassifier(object):
    ...
    def _score(self, loader, indices, k):
        def eval_score_perclass(jacob, data):
            ...
            try:
                K = 1e-3
                score = sum(list(np.absolute(list(map(lambda i: np.sum(np.log(np.absolute(np.corrcoef(jacob[labels==i].view(labels.size(0), -1).cpu().numpy()+K))+K))/1e+5,list(torch.unique(labels)))))))
            except:
                return 0
            return score
        data = next(iter(loader))
        j_d = self._get_jacobian(data, indices, k)
        return eval_score_perclass(**j_d)/1e+5 #sum(list(map(lambda data:eval_score_perclass(j_d["J"], data["labels"])/1e+2, loader)))
