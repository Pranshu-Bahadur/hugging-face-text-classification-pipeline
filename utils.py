...
class SpreadSheetNLPCustomDataset(Dataset):
    def __init__(self, csv_path, tokenizer, library, long):
        ...
        cols_n.reverse()
        types = list(self.dataset.type.unique()) + list(map(lambda t: t.lower(),list(self.dataset.type.unique())))
        ...
        self.dataset = self.dataset[~self.dataset['posts'].str.lower().isin(types)]
