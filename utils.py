...
class SpreadSheetNLPCustomDataset(Dataset):
    def __init__(self, csv_path, tokenizer, library, long):
        ...
        self.dataset = self.dataset[self.dataset['posts'].map(len) >= 256//4]
        self.dataset = self.dataset[self.dataset.posts.map(lambda phrase:~self.dataset['posts'].str.contains(phrase), filter_links_phrases).all()]
