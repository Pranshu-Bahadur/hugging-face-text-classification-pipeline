...
class SpreadSheetNLPCustomDataset(Dataset):
    def __init__(self, csv_path, tokenizer, library, long):
        ...
        self.dataset = pd.DataFrame(pd.concat([Series(row['type'], list(filter(lambda p: len(p) >= 512//4 and all(list(map(lambda phrase: not phrase in p,filter_links_phrases))), row['posts'].split("|||")))) for _, row in self.dataset.iterrows()]).reset_index())
        [self.dataset.rename(columns = {name:cols_n[i]}, inplace = True) for i,name in enumerate(self.dataset.columns.tolist())]
        self.encodings = tokenizer(list(self.dataset['posts'].values), padding='max_length', max_length=512)
